...
if __name__ == '__main__':
    ...
    for spectrum in ['fk_m', 'fk_cb', 'fk_weyl']:
        ...
        plt.plot(
            ref_k,
            np.abs(fk_emu[0, 0]/fk_class_2[0, 0] - 1.)*100.,
            label='HiFast Emu/Class(high prec)')
        plt.plot(
            ref_k,
            np.abs(fk_class_0[0, 0]/fk_class_2[0, 0] - 1.)*100.,
            label='Class(low prec)/Class(high prec)')
        plt.plot(
            ref_k,
            np.abs(fk_class_1[0, 0]/fk_class_2[0, 0] - 1.)*100.,
            label='Class(std prec)/Class(high prec)')
        plt.plot(
            ref_k,
            np.abs(fk_data/fk_class_2[0, 0] - 1.)*100.,
